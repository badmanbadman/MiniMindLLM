...
from dataset.novel_lm_dataset import NovelLoRADataset
...
def train_epoch(epoch,loader, iters, lora_patams, start_step=0,wandb=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="../dataset/processed_data/lora_training_data.jsonl", help="LoRA训练数据路径")
    ...
    Logger(f"LoRA总参数量：{lora_params_count / 1e6:.3f} M")
    ...
    #6 定义数据和优化器
    train_ds = NovelLoRADataset(args.data_path, tokenizer,max_length=args.max_seq_len)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch == start_epoch and start_step > 0: #第一个epoch且存在检测点
