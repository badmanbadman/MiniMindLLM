...
def train_epoch(epoch, loader, iters, tokenizer,lm_config, start_step=0,wandb=None):
    ...
    for step, (X, Y, loss_mask) in enumerate(loader, start=start_step +1):
        ...
        with autocast_ctx:
            ...
            # 找出特殊位置的ids
            """集合操作函数 检查一个张量中的元素是否存在于另一个张量中
            torch.isin(elements,test_elements, *, assume_unique=False, invert=False)
            elements: 要检查的张量
            test_element: 用于测试元素是否在其中的张量
            assume_unique: 如果维True,则假设2个张量都是唯一的，可以加快计算速度
            invert：如果为true，则返回不再test_elements中的元素（即取反）
            返回一个与elements形状相同的布尔张量，如果elements中的元素在test_elements中，则对应位置为True，否则为False
            示例：
            import torch
            ...
            Y.view(-1) 是将标签张量 Y 展平为一维张量。
            torch.tensor(...) 创建了一个包含所有特殊标记ID的张量，然后移动到设备上。
            torch.isin 返回一个布尔张量，表示 Y 中每个位置对应的标签ID是否在特殊标记ID集合中。
            然后，这个布尔张量 sp_ids 被用来对 loss_mask 中对应位置赋予更高的权重（10倍），从而在损失函数中强调这些特殊标记。
            """
            sp_ids = torch.isin(
                Y.view(-1),
                torch.tensor(
                    start_of_think_ids+end_of_think_ids+start_of_answer_ids+end_of_answer_ids).to(args.device)
                    )
            loss_mask = loss_mask.view(-1)
            """
            布尔索引复制操作
            sp_ids ： 布尔掩码，标记哪些位置为特殊标记
            loss_mask[sp_ids]=10 对掩码位置为True的位置赋值为10


            # 原始loss_mask（假设形状为[2, 4]）
            loss_mask = torch.tensor([[1, 1, 1, 1],
                                    [1, 1, 1, 1]], dtype=torch.float32)

            # sp_ids布尔掩码（已展平为一维）
            sp_ids = torch.tensor([False, True, False, True, 
                                False, True, False, False])

            loss_mask_flat = loss_mask.view(-1)
            # loss_mask_flat: tensor([1., 1., 1., 1., 1., 1., 1., 1.])
            ...
            当sp_ids[i] == True时，将loss_mask_flat[i]设置为10

            当sp_ids[i] == False时，保持原值不变

            # 赋值后的loss_mask_flat
            loss_mask_flat = tensor([ 1., 10.,  1., 10.,  1., 10.,  1.,  1.])
            loss_mask = loss_mask_flat.view(loss_mask.shape)
            # loss_mask: tensor([[ 1., 10.,  1., 10.],
            #                    [ 1., 10.,  1.,  1.]])
            """
            loss_mask_sum = loss_mask.sum()
            loss_mask[sp_ids] = 10  # 对思考标签增加10倍的权重
