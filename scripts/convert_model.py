...
# MoE模型需使用此函数转换
def convert_torch2transformers_minimind(lm_config,torch_path, transformers_path, dtype=torch.float16):
...
def convert_torch2transformers_llama(lm_config,torch_path, transformers_path, dtype=torch.float16):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    state_dict = torch.load(torch_path, map_location=device)
